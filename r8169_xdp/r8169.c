				      ...
				      struct ethtool_stats *stats, u64 *data)
{
	...
	data[13] = tp->xdp_counter.set;
	data[14] = tp->xdp_counter.pass;
	data[15] = tp->xdp_counter.drop;
	data[16] = tp->xdp_counter.tx;
	data[17] = tp->xdp_counter.abort;
	data[18] = tp->xdp_counter.unknown;
    data[19] = tp->xdp_counter.redirect;
}
...
static int rtl_xdp_xmit(struct net_device *dev, struct xdp_buff *xdp);
					   ...
					   u64 dma_addr)
{
	...
	dma_sync_single_for_cpu(d, dma_addr, pkt_size, DMA_FROM_DEVICE);
    cpu_addr = (u64)phys_to_virt(dma_addr);
    ...
    case XDP_REDIRECT:
        ...
        if (err) {
        printk(KERN_DEBUG"xdp do redirect%d\n", err);
...
pass_out:
	...
	dma_sync_single_for_device(d, dma_addr, pkt_size, DMA_FROM_DEVICE);
...
process_pkt:
			dma_addr = le64_to_cpu(desc->addr);    /* 大小端转换 */
...
static int rtl_xdp_setup(struct net_device *dev, struct bpf_prog *prog)
{
    struct rtl8169_private *tp = netdev_priv(dev);
    ...
    if (tp->xdp_prog) {
        /* 附上bpf程序 */
        tp->xdp_prog = bpf_prog_add(prog, 1);
    }
    /* bpf程序计数 */
    tp->xdp_counter.set++;
    rtl_unlock_work(tp);
...
static int rtl_xdp(struct net_device *dev, struct netdev_bpf *xdp)
{
    ...
    switch(xdp->command) {
    case XDP_SETUP_PROG:
        return rtl_xdp_setup(dev, xdp->prog);
    case XDP_QUERY_PROG:
        xdp->prog_attached = !!tp->xdp_prog;
        xdp->prog_id = tp->xdp_prog ? tp->xdp_prog->aux->id : 0;
        return 0;
    default:
        return -EINVAL;
    } 
...
static int __rtl_xdp_xmit(struct net_device *dev, struct xdp_buff *xdp)
{
    struct rtl8169_private *tp = netdev_priv(dev);
    unsigned int entry = tp->cur_tx % NUM_TX_DESC;
    struct TxDesc *txd = tp->TxDescArray + entry;
    ...
    mapping = dma_map_single(d, xdp->data, len, DMA_TO_DEVICE);
    if (unlikely(dma_mapping_error(d, mapping))) {
        if (net_ratelimit())
            netif_err(tp, drv, dev, "XDP failed to map TX DMA!\n");
        goto err_dma_0;
    }
    
    tp->tx_skb[entry].len = len;
    txd->addr = cpu_to_le64(mapping);
    tp->tx_skb[entry].data = xdp->data;
...
err_dma_0:
    tp->xdp_counter.drop++;
    return R8169_XDP_DROP;
}

static int rtl_xdp_xmit(struct net_device *dev, struct xdp_buff *xdp)
{
    int err, running = netif_running(dev);
...
static void rtl_xdp_flush(struct net_device *dev)
{
    struct rtl8169_private *tp = netdev_priv(dev);
    ...
    tp->cur_tx = tp->cur_tx % NUM_TX_DESC - 1; 
    wmb();
